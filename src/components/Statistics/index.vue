<template>
    <el-card class="template-card content-details statistics-list">
        ...
        <el-form label-width="80px" :inline="true">
            <el-form-item label="场所" v-if="$route.query.source != 'place' && $route.query.source != 'screen'">
                <el-select
                    v-model="params.placeId"
                    filterable
                    clearable
                    placeholder="请选择场所"
                    @change="$delete(params, 'screenId');search();getScreen()"
                >
                    <el-option
...
import { placeGameList, placeScreenList, placeStatistics, placeStatisticsDetail } from '@/api/place';
import { screenGameList, screenStatistics, screenStatisticsDetail } from '@/api/screen';
import { contentPlaceList, contentScreenList, contentStatistics, contentStatisticsDetail } from '@/api/content';
...
export default {
    ...
    created() {
        ...
        this.init();
        if(this.$route.query.source != 'place' && this.$route.query.source != 'screen') this.getPlaceList();
    ...
    methods: {
        ...
        //导出
        download(){
            window.open(`${document.location.origin}/${this.$route.query.source}/export?${qs.stringify(this.params)}`);
        }
    },
    components: {
        StatisticsDetail,
