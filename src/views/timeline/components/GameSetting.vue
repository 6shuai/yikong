<template>
<!-- 变更游戏包版本 -->
<el-dialog class="game-setting" title="游戏设置" :modal-append-to-body="false" :visible.sync="gameSettingDialog" :close-on-click-modal="false" :close-on-press-escape="false" width="600px">
    <div class="tab-wrap">
    ...
    <div v-loading="dataLoading" v-if="activeName=='packageVersion'" class="game-setting-content">
        <el-card class="type-card" v-for="(item, index) in packageVersionData" :key="index" :class="{'active':params.packageId==item.id}" @click.native="selectedCurrentVersion(item.id)">
    ...
    <div v-if="activeName=='config'" class="game-setting-content">
        <el-card class="type-card" v-for="(item, index) in gameConfigData" :key="index" :class="{'active':params.configId==item.id}" @click.native="selectedCurrentConfig(item.id)">
    ...
    methods: {
        ...
        handleSaveConfig() {
            if (!this.params.packageId) {
                ...
                return
            } else if (!this.params.configId && !this.gameConfigData.length) {
